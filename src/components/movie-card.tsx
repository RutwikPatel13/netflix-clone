...
export function MovieCard({ movie, priority = false }: MovieCardProps) {
  ...
  return (
    ...
    >
      {/* Movie Poster - using explicit width/height instead of fill */}
      <div className="relative w-full overflow-hidden rounded-md bg-netflix-gray">
        <Image
          src={imageError ? '/placeholder-movie.jpg' : getPosterUrl(movie.poster_path)}
          alt={movie.title}
          width={500}
          height={750}
        ...
        >
          ...
          <div className="space-y-1">
            <h3 className="line-clamp-1 text-xs font-semibold">{movie.title}</h3>
